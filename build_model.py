...
class EncoderRNN(nn.Module):
    """
    A bidirectional RNN. It takes token IDs and outputs state vectors of every time step.
    """
    def __init__(self, n_words, hidden_size, num_layers, use_bn):
        """
        Args:
        ...
        if use_bn:
            self.bns = nn.ModuleList([nn.BatchNorm1d(2 * hidden_size) for _ in range(num_layers-1)])
    ...
    def forward(self, xs):
        ...
        Args:
            xs (torch.LongTensor, [batch_size, seq_length]): A mini-batch of word IDs.

        Returns:
            xs (PackedSequence): The packed output states.
        """
        batch_size = xs.shape[0]
        xlens = xs.gt(0).sum(dim=-1)   # [batch_size]
        xs = self.embed(xs)            # [batch_size, seq_length, hidden_size]
    ...
    def apply_bn(self, xs, layer_id):
        ...
        """
        if layer_id == 0:
            # We won't add BatchNorm at word embedding vectors.
            return xs
        ...
        # Apply BatchNorm
        xs = self.bns[layer_id-1](xs)   # [total_seq_length, C]
        # Repack
...
class Seq2Seq(nn.Module):
    ...
    """
    def __init__(self, source_size, target_size, hidden_size, encoder_layers, decoder_layers, drop_p=0., use_bn=True):
        """
        Args:
            source_size (integer): Source vocabulary size.
        ...
        self.encoder = EncoderRNN(source_size, hidden_size, encoder_layers, use_bn)
        self.decoder = DecoderRNN(target_size, hidden_size, decoder_layers, drop_p)

    def forward(self, xs, ys=None, beam_width=1):
        """
        The forwarding behavior depends on if ground-truths (ys) are provided.

        Args:
            xs, ys (torch.LongTensor, [batch_size, seq_length]): Source/target sequences.
            beam_width (integer): Beam Search width. Beam Search is equivalent to Greedy Search when beam_width=1.

        Returns:
            * When ground-truths (ys) are provided, it returns cross-entropy loss. Otherwise it returns predicted
            word IDs and the attention weights.
        ...
        """
        if ys is None:
            predictions, attn_weights = self.decoder(self.encoder(xs), beam_width=beam_width)
            return predictions, attn_weights
        else:
            loss = self.decoder(self.encoder(xs), ys)
            return loss
